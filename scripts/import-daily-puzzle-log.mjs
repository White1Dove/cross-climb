#!/usr/bin/env node

import fs from "node:fs";
import path from "node:path";
import process from "node:process";

const ROOT = process.cwd();
const CURRENT_CROSSCLIMB_PATH = path.join(ROOT, "data", "current-crossclimb.json");
const CROSSCLIMB_HISTORY_PATH = path.join(ROOT, "data", "crossclimb-history.json");
const CURRENT_PINPOINT_PATH = path.join(ROOT, "data", "current-pinpoint.json");
const PINPOINT_HISTORY_PATH = path.join(ROOT, "data", "pinpoint-history.json");
const PUBLISHABLE_GAME_IDS = new Set(["crossclimb", "pinpoint"]);
const CROSSCLIMB_ROW_POSITIONS = new Set(["top", "middle", "bottom"]);

function usage() {
  console.error("Usage: pnpm import:puzzle-log <log.json | ->");
  console.error("       Use '-' to read a copied log JSON from stdin.");
}

function readJsonInput(inputPath) {
  const raw = inputPath === "-"
    ? fs.readFileSync(0, "utf8")
    : fs.readFileSync(path.resolve(ROOT, inputPath), "utf8");
  return JSON.parse(raw);
}

function unwrapInput(input) {
  if (input?.log && typeof input.log === "object") {
    return input.log;
  }

  return input;
}

function normalizeWord(value) {
  return String(value || "")
    .replace(/[^a-z]/gi, "")
    .toUpperCase();
}

function normalizeWords(values) {
  return Array.isArray(values) ? values.map(normalizeWord).filter(Boolean) : [];
}

function normalizeText(value) {
  return String(value || "").replace(/\s+/g, " ").trim();
}

function getCrossclimbExplanation(...sources) {
  return sources
    .flatMap((source) => [
      source?.explanation,
      source?.why_it_fits,
      source?.reasoning,
      source?.analysis,
    ])
    .map((value) => (typeof value === "string" ? normalizeText(value) : ""))
    .find(Boolean);
}

function formatDisplayDate(value) {
  const text = String(value || "").trim();
  const isoMatch = text.match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (!isoMatch) {
    return text;
  }

  const [, year, month, day] = isoMatch;
  const date = new Date(Date.UTC(Number(year), Number(month) - 1, Number(day), 12, 0, 0));
  return new Intl.DateTimeFormat("en-US", {
    month: "long",
    day: "numeric",
    year: "numeric",
    timeZone: "UTC",
  }).format(date);
}

function formatIsoDate(value) {
  const text = String(value || "").trim();
  const isoMatch = text.match(/^(\d{4})-(\d{2})-(\d{2})$/);

  if (isoMatch) {
    return text;
  }

  const date = new Date(`${text} 12:00:00 UTC`);
  if (Number.isNaN(date.getTime())) {
    return "";
  }

  return date.toISOString().slice(0, 10);
}

function getPuzzleNumber(log) {
  const raw = log.puzzle_number || log.normalized_puzzle?.issue_number || "";
  const parsed = Number(raw);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : 0;
}

function choosePublishableLog(input) {
  const rawLogs = Array.isArray(input) ? input : [unwrapInput(input)];
  const candidates = rawLogs
    .map(unwrapInput)
    .filter((log) => PUBLISHABLE_GAME_IDS.has(log?.game_id))
    .sort((a, b) => String(b.captured_at || "").localeCompare(String(a.captured_at || "")));

  if (candidates.length === 0) {
    throw new Error("No publishable Crossclimb or Pinpoint log found in input.");
  }

  const valid = candidates.find((log) => log.validation?.is_valid !== false && log.status !== "conflict");
  if (!valid) {
    throw new Error("Only invalid or conflict logs were found; refusing to publish.");
  }

  return valid;
}

function getFullLadder(log) {
  const fullLadder = normalizeWords(log.solution?.full_ladder);
  if (fullLadder.length > 0) {
    return fullLadder;
  }

  const ladder = normalizeWords(log.solution?.ladder);
  const topWord = normalizeWord(log.solution?.top_word);
  const bottomWord = normalizeWord(log.solution?.bottom_word);

  if (ladder.length === 0) {
    return [topWord, bottomWord].filter(Boolean);
  }

  const ladderAlreadyIncludesEdges = (!topWord || ladder[0] === topWord) && (!bottomWord || ladder.at(-1) === bottomWord);
  if (ladderAlreadyIncludesEdges) {
    return ladder;
  }

  return [topWord, ...ladder, bottomWord].filter(Boolean);
}

function getPosition(index, total) {
  if (index === 0) return "top";
  if (index === total - 1) return "bottom";
  return "middle";
}

function findCrossclimbSourceByWord(items, word) {
  return Array.isArray(items)
    ? items.find((item) => normalizeWord(item?.word) === word) || {}
    : {};
}

function buildCrossclimbRows(log, fullLadder) {
  const sourceRows = Array.isArray(log.normalized_puzzle?.rows) ? log.normalized_puzzle.rows : [];
  const sourceClues = Array.isArray(log.normalized_puzzle?.clues) ? log.normalized_puzzle.clues : [];
  const finalClue = normalizeText(log.normalized_puzzle?.final_clue || log.raw_puzzle?.final_clue || "");

  return fullLadder.map((word, index) => {
    const sourceRow = findCrossclimbSourceByWord(sourceRows, word);
    const sourceClue = findCrossclimbSourceByWord(sourceClues, word);
    const position = getPosition(index, fullLadder.length);
    const clue = normalizeText(
      sourceRow.clue ||
      sourceClue.text ||
      sourceClue.clue ||
      ((position === "top" || position === "bottom") ? finalClue : "")
    );

    const explanation = getCrossclimbExplanation(sourceRow, sourceClue);
    const row = {
      index: index + 1,
      position,
      clue,
      word,
      answer_length: word.length,
    };

    return explanation ? { ...row, explanation } : row;
  });
}

function buildCrossclimbData(log) {
  const fullLadder = getFullLadder(log);
  if (fullLadder.length < 2) {
    throw new Error("Crossclimb log does not contain enough ladder words.");
  }

  const rows = buildCrossclimbRows(log, fullLadder);
  const wordLength = fullLadder[0]?.length || Number(log.normalized_puzzle?.word_length) || 0;
  const middleLadder = fullLadder.slice(1, -1);
  const puzzleNumber = getPuzzleNumber(log);
  const puzzleDate = formatDisplayDate(log.puzzle_date);
  const finalClue = normalizeText(
    log.normalized_puzzle?.final_clue ||
      log.raw_puzzle?.final_clue ||
      rows.find((row) => row.position === "top")?.clue ||
      rows.find((row) => row.position === "bottom")?.clue ||
      ""
  );
  const missingClueRows = rows.filter((row) => !row.clue);

  if (!puzzleNumber) {
    throw new Error("Crossclimb log is missing puzzle_number; refusing to publish ambiguous content.");
  }

  if (missingClueRows.length > 0) {
    console.warn(
      `Crossclimb log is missing clues for row(s): ${missingClueRows
        .map((row) => row.index)
        .join(", ")}. Current data will be updated, but new history rows will be skipped; existing complete history rows for the same ladder may be preserved.`
    );
  }

  return {
    game_id: "crossclimb",
    game_name: "Crossclimb",
    puzzle_number: puzzleNumber,
    puzzle_date: puzzleDate,
    puzzle_label: log.puzzle_label || `Crossclimb #${puzzleNumber}`,
    normalized_puzzle: {
      word_count: fullLadder.length,
      word_length: wordLength,
      middle_word_count: middleLadder.length,
      ...(finalClue ? { final_clue: finalClue } : {}),
      rows,
    },
    hints: {
      no_spoiler_hints: [
        `${fullLadder.length} rows`,
        wordLength ? `${wordLength} letters each` : "Same-length words",
        `${middleLadder.length} middle words`,
      ],
      medium_hints: rows.map((row) => `Starts with ${row.word[0]}`),
    },
    solution: {
      top_word: fullLadder[0],
      bottom_word: fullLadder.at(-1),
      full_ladder: fullLadder,
      middle_ladder: middleLadder,
    },
  };
}

function readCrossclimbHistory() {
  if (!fs.existsSync(CROSSCLIMB_HISTORY_PATH)) {
    return [];
  }

  return JSON.parse(fs.readFileSync(CROSSCLIMB_HISTORY_PATH, "utf8"));
}

function sameCrossclimbLadder(left, right) {
  const leftWords = normalizeWords(left);
  const rightWords = normalizeWords(right);
  return leftWords.length === rightWords.length && leftWords.every((word, index) => word === rightWords[index]);
}

function getCompleteCrossclimbRows(rows, fullLadder) {
  if (!Array.isArray(rows) || rows.length !== fullLadder.length) {
    return null;
  }

  const normalizedRows = rows.map((row, index) => {
    const word = normalizeWord(row?.word);
    const clue = normalizeText(row?.clue);
    const position = String(row?.position || "").trim().toLowerCase();
    const answerLength = Number(row?.answer_length);
    const expectedPosition = getPosition(index, fullLadder.length);

    if (
      Number(row?.index) !== index + 1 ||
      !CROSSCLIMB_ROW_POSITIONS.has(position) ||
      position !== expectedPosition ||
      word !== fullLadder[index] ||
      !clue ||
      !Number.isFinite(answerLength) ||
      answerLength !== word.length
    ) {
      return null;
    }

    const explanation = getCrossclimbExplanation(row);

    return {
      index: index + 1,
      position,
      clue,
      word,
      answer_length: word.length,
      ...(explanation ? { explanation } : {}),
    };
  });

  return normalizedRows.every(Boolean) ? normalizedRows : null;
}

function updateCrossclimbHistory(puzzle, sourceDate) {
  const history = readCrossclimbHistory();
  const fullLadder = puzzle.solution.full_ladder;
  const existing = history.find((item) => Number(item.number) === Number(puzzle.puzzle_number));
  const rowsForHistory =
    getCompleteCrossclimbRows(puzzle.normalized_puzzle?.rows, fullLadder) ||
    (existing && sameCrossclimbLadder(existing.ladder, fullLadder)
      ? getCompleteCrossclimbRows(existing.rows, fullLadder)
      : null);
  const entry = {
    number: puzzle.puzzle_number,
    date: formatDisplayDate(sourceDate || puzzle.puzzle_date),
    isoDate: formatIsoDate(sourceDate || puzzle.puzzle_date),
    start: fullLadder[0],
    end: fullLadder.at(-1),
    ladder: fullLadder,
    ...(rowsForHistory ? { rows: rowsForHistory } : {}),
  };

  const next = [entry, ...history.filter((item) => Number(item.number) !== Number(entry.number))]
    .filter((item) => Number.isFinite(Number(item.number)) && item.number > 0)
    .sort((a, b) => Number(b.number) - Number(a.number));

  writeJson(CROSSCLIMB_HISTORY_PATH, next);
}

function getPinpointAnswer(log) {
  return normalizeText(
    log.solution?.final_answer ||
    log.normalized_puzzle?.category ||
    log.hints?.answer_reveal?.final_answer ||
    log.raw_puzzle?.answer
  );
}

function normalizePinpointClues(values) {
  if (!Array.isArray(values)) {
    return [];
  }

  const seen = new Set();
  return values
    .map((clue, index) => ({
      index: Number(clue?.index) || index + 1,
      text: normalizeText(clue?.text || clue?.value || clue),
    }))
    .filter((clue) => {
      const key = clue.text.toLowerCase();
      if (!clue.text || seen.has(key)) {
        return false;
      }
      seen.add(key);
      return true;
    })
    .map((clue, index) => ({
      index: index + 1,
      text: clue.text,
    }));
}

function getPinpointClues(log) {
  const normalizedClues = normalizePinpointClues(log.normalized_puzzle?.clues);
  return normalizedClues.length > 0
    ? normalizedClues
    : normalizePinpointClues(log.hints?.answer_reveal?.clues || log.raw_puzzle?.clues);
}

function buildPinpointData(log) {
  const puzzleNumber = getPuzzleNumber(log);
  const puzzleDate = formatDisplayDate(log.puzzle_date);
  const finalAnswer = getPinpointAnswer(log);
  const clues = getPinpointClues(log);

  if (!puzzleNumber) {
    throw new Error("Pinpoint log is missing puzzle_number; refusing to publish ambiguous content.");
  }

  if (!finalAnswer) {
    throw new Error("Pinpoint log is missing solution.final_answer.");
  }

  if (clues.length === 0) {
    throw new Error("Pinpoint log is missing normalized clues.");
  }

  return {
    game_id: "pinpoint",
    game_name: "Pinpoint",
    puzzle_number: puzzleNumber,
    puzzle_date: puzzleDate,
    puzzle_label: log.puzzle_label || `Pinpoint #${puzzleNumber}`,
    normalized_puzzle: {
      category: finalAnswer,
      clue_count: clues.length,
      clues,
    },
    hints: {
      no_spoiler_hints: [`Today's Pinpoint has ${clues.length} clues.`],
      medium_hints: clues[0] ? [`First clue: ${clues[0].text}`] : [],
    },
    solution: {
      final_answer: finalAnswer,
    },
    ...(log.analysis && typeof log.analysis === "object" ? { analysis: log.analysis } : {}),
  };
}

function readPinpointHistory() {
  if (!fs.existsSync(PINPOINT_HISTORY_PATH)) {
    return [];
  }

  return JSON.parse(fs.readFileSync(PINPOINT_HISTORY_PATH, "utf8"));
}

function updatePinpointHistory(puzzle, sourceDate) {
  const history = readPinpointHistory();
  const entry = {
    number: puzzle.puzzle_number,
    date: formatDisplayDate(sourceDate || puzzle.puzzle_date),
    isoDate: formatIsoDate(sourceDate || puzzle.puzzle_date),
    answer: puzzle.solution.final_answer,
    clues: puzzle.normalized_puzzle.clues.map((clue) => clue.text),
    ...(puzzle.analysis ? { analysis: puzzle.analysis } : {}),
  };

  const next = [entry, ...history.filter((item) => Number(item.number) !== Number(entry.number))]
    .filter((item) => Number.isFinite(Number(item.number)) && item.number > 0)
    .sort((a, b) => Number(b.number) - Number(a.number));

  writeJson(PINPOINT_HISTORY_PATH, next);
}

function writeJson(filePath, value) {
  fs.writeFileSync(filePath, `${JSON.stringify(value, null, 2)}\n`);
}

function main() {
  const inputPath = process.argv[2];
  if (!inputPath) {
    usage();
    process.exit(1);
  }

  const input = readJsonInput(inputPath);
  const log = choosePublishableLog(input);

  if (log.game_id === "crossclimb") {
    const puzzle = buildCrossclimbData(log);
    writeJson(CURRENT_CROSSCLIMB_PATH, puzzle);
    updateCrossclimbHistory(puzzle, log.puzzle_date);
    console.log(`Imported ${puzzle.puzzle_label} (${puzzle.puzzle_date}) into data/current-crossclimb.json`);
    return;
  }

  if (log.game_id === "pinpoint") {
    const puzzle = buildPinpointData(log);
    writeJson(CURRENT_PINPOINT_PATH, puzzle);
    updatePinpointHistory(puzzle, log.puzzle_date);
    console.log(`Imported ${puzzle.puzzle_label} (${puzzle.puzzle_date}) into data/current-pinpoint.json`);
    return;
  }

  throw new Error(`Unsupported game_id: ${log.game_id}`);
}

main();
